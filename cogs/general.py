...
class General(commands.Cog, name="general"):
    ...
    @app_commands.command(
        name="setname",
        description="Mengubah Nama Kalian berdasarkan nickname in game minecraft kalian",
    )
    async def setname(self, context: Context, *, name: str) -> None:
        try:
            nama_player = context.message.author.nick
            new_player_name = nama_player + f"| {name}"
            await context.message.author.edit(nick=new_player_name)
            await context.message.channel.send(
                embed=discord.Embed(
                    title="Pengubahan Nama",
                    description=f"Nama lama: {nama_player}"
                                f"Nama baru: {new_player_name}",
                    color=0xBEBEFE
                )
            )
            ...
            await context.message.channel.send(
                embed=discord.Embed(
                    title="Pengubahan Nama",
                    description="Terjadi eror saat mengubah nama, pastikan saya memiliki izin untuk mengubah nickname kalian dan coba lagi nanti." \
                    "Error: " + str(e),
                )
            )
