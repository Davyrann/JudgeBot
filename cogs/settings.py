...
class Settings(commands.Cog, name="Settings"):
    ...
    team_group = app_commands.Group(
        name="team",
        parent=settings_group,
        description="Perintah untuk mengelola pengaturan tim",
        )
    
    async def team_settings_autocomplete(self, interaction: Interaction, current: str) -> list[app_commands.Choice[str]]:
        settings = {"Team Log Channel": "team_log_channel_id",
                    "Player Log Channel": "player_log_channel_id"}
        return [
            app_commands.Choice(name=label, value=tech_key) 
            for label, tech_key in settings.items() 
            if current.lower() in label.lower()
    ...
    async def set_team_channel(self, interaction: Interaction, setting: str, channel: TextChannel):
        ...
        if "team" not in self.bot.guild_settings_cache[guild_id]:
            self.bot.guild_settings_cache[guild_id]["team"] = cast(models.TeamSettings, {})
